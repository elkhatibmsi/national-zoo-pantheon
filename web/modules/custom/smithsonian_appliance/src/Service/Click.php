...
/**
 * @file
 * The callback for the Smithsonian Appliance module's /click service.
 *
 ...
 */
function _smithsonian_appliance_shift_path_up($path) {
  if (empty($path)) {
    return FALSE;
  }
  $path = explode('/', $path);
 ...
 */
function smithsonian_appliance_valid_drupal_root($path) {
 ...
 */
function smithsonian_appliance_locate_root($start_path = NULL) {
  $drupal_root = FALSE;
  ...
  foreach ([TRUE, FALSE] as $follow_symlinks) {
    ...
    if (smithsonian_appliance_valid_drupal_root($path)) {
      $drupal_root = $path;
      break;
    }
    else {
      // Move up dir by dir and check each.
      while ($path = _smithsonian_appliance_shift_path_up($path)) {
        if ($follow_symlinks && is_link($path)) {
          $path = realpath($path);
        }
        if (smithsonian_appliance_valid_drupal_root($path)) {
...
define('DRUPAL_ROOT', smithsonian_appliance_locate_root());
...
$gsa_hostname = variable_get('smithsonian_appliance_hostname', FALSE);
$gsa_timeout = variable_get('smithsonian_appliance_timeout', 10);

// Make sure we have a valid GSA host to connect to.
if (!$gsa_hostname) {
  watchdog('smithsonian_appliance', 'No valid GSA host to connect to.', [], WATCHDOG_ERROR);
  exit;
}

// Filter parameters.
// @see https://www.smithsonian.com/support/enterprise/static/gsa/docs/admin/70/gsa_doc_set/xml_reference/advanced_search_reporting.html#1080237
$parameters_allowed = [
  'cd' => 1,
  'ct' => 1,
  'q' => 1,
...
// @see https://www.smithsonian.com/support/enterprise/static/gsa/docs/admin/70/gsa_doc_set/xml_reference/advanced_search_reporting.html#1080347
switch ($result->code) {
  case 204:
  ...
  default:
    ...
    watchdog('smithsonian_appliance', $message, $vars, WATCHDOG_WARNING);
}
