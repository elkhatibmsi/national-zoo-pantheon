...
class AdministrationForm extends ConfigFormBase {
  ...
  public function getFormId() {
    return 'smithsonian_appliance_admin_settings';
  }
  ...
  protected function getEditableConfigNames() {
    return 'smithsonian_appliance.settings';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['connection_info']['hostname'] = [
      '#type' => 'url',
      '#title' => $this->t('Smithsonian Search Appliance Host Name'),
      ...
      '#description' => $this->t('Learn more about GSA auto-filtering @gsa_link. In general, employing both filters enhances results, but sites with smaller indexes may suffer from over-filtered results.',
        [
          '@gsa_link' => Link::fromTextAndUrl(
            $this->t('here'),
            Url::fromUri('http://code.smithsonian.com/apis/searchappliance/documentation/68/xml_reference.html#request_filter_auto')
          )->toString(),
        ]
      ),
    ];
      ...
      '#description' => $this->t('Learn more about @this_feature. You need to enable Advanced Search Reporting on the front end client. The device should provide a file named "/clicklog_compiled.js" when using the search interface on the GSA.',
        [
          '@this_feature' => Link::fromTextAndUrl(
            $this->t('this feature'),
            Url::fromUri('http://www.smithsonian.com/support/enterprise/static/gsa/docs/admin/70/gsa_doc_set/xml_reference/advanced_search_reporting.html')
          )->toString(),
        ]
      ),
    ];
    $form['display_settings']['sitelinks_search_box'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('Enable Smithsonian.com Sitelinks Search Box'),
      '#default_value' => $settings['display_settings']['sitelinks_search_box'],
      '#description' => $this->t(
        'Learn more about @this_feature. Note: your site may not necessarily be a candidate for the smithsonian.com Sitelinks search box.',
        [
          '@this_feature' => Link::fromTextAndUrl(
           $this->t('this feature'),
            Url::fromUri('https://developers.smithsonian.com/structured-data/slsb-overview')
          )->toString(),
        ]
      ),
    ];
