...
use Drupal\smithsonian_appliance\SearchResults\KeyMatch;
use Drupal\smithsonian_appliance\SearchResults\OneBoxResult;
use Drupal\smithsonian_appliance\SearchResults\OneBoxResultSet;
...
class Parser implements ParserInterface {
   ...
   * Cached parse result.
   *
   * @var null|\Drupal\smithsonian_appliance\SearchResults\ResultSet
   */
  protected $response;
  ...
  public function parseResponse($xml, $useCached = TRUE) {
    if (!$this->response || FALSE === $useCached) {
      ...
      if ($payload === FALSE) {
        ...
        // @see http://code.smithsonian.com/apis/searchappliance/documentation/50/xml_reference.html#tag_onesynonym
        ...
        $this->moduleHandler->alter('smithsonian_appliance_results', $results, $payload);
        $this->response = $response;
      }
    }
   ...
   *   Payload.
   * @param \Drupal\smithsonian_appliance\SearchResults\ResultSet $response
   *   Response.
   */
  ...
  protected function parseResultEntries(\SimpleXMLElement $payload, ResultSet $response) {
    foreach ($payload->xpath('//R') as $res) {
      ...
      // @see smithsonian-appliance-result.tpl.php
  ...
  protected function parseOneboxResults(\SimpleXMLElement $payload, ResultSet $response) {
    // Onebox results.
    // @see https://developers.smithsonian.com/search-appliance/documentation/614/oneboxguide#providerresultsschema
    // @see https://developers.smithsonian.com/search-appliance/documentation/614/oneboxguide#mergingobs
