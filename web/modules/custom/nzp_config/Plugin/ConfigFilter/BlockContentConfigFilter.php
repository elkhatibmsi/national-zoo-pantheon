 ...
 *
 * @ConfigFilter(
 *   id = "config_block_content_ignore",
 *   label =  @Translation("A plugin to filter out all Block Content configs"),
 *   weight = 100
 * )
 */
class BlockContentPluginFilter extends ConfigFilterBase implements ContainerFactoryPluginInterface {
  ...
  public function __construct(array $configuration, $plugin_id, array $plugin_definition, StorageInterface $active) {
    ...
    $this->active = $active;
    $this->disabled = Settings::get('block_content_config_ignore_disabled', TRUE);
  }
  ...
  /**
   * Match a config entity name against the block_content config entities.
   *
  ...
  protected function matchConfigName($config_name) {
    ...
    if (strpos($config_name, 'block.block.') === 0) {
      return TRUE;
    }
  ...
  protected function activeRead($name, $data) {
    ...
    // If the active block_content data cannot be found, return data to allow
    // imports of new block_contents.
    if ($active_data) {
      return $active_data;
    }
  ...
  public function filterListAll($prefix, array $data) {
    ...
    // Filter out only block_content config names.
